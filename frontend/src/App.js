...
const LiquidGlass = ({ isVisible, onToggle }) => {
    ...
    class Shader {
      ...
      createElement() {
        ...
        this.container.style.cssText = `
          ...
          overflow: hidden;
          border-radius: 20px;
          box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3), 0 -10px 25px inset rgba(0, 0, 0, 0.15);
          cursor: grab;
  ...
  return (
    <div ref={containerRef}>
      ...
      <motion.button
        onClick={onToggle}
        className="fixed top-4 right-4 z-[1001] bg-gradient-to-r from-blue-600 to-purple-600 text-white px-4 py-2 rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
      ...
      >
        {isVisible ? '🔒 Hide Glass' : '💎 Show Glass'}
      </motion.button>
    </div>
  );
};
...
const App = () => {
  ...
  const [liquidGlassVisible, setLiquidGlassVisible] = useState(false);
  ...
  return (
    <div className="relative min-h-screen bg-gray-900 text-white overflow-x-hidden">
      {/* Liquid Glass Effect */}
      <LiquidGlass 
        isVisible={liquidGlassVisible} 
        onToggle={() => setLiquidGlassVisible(!liquidGlassVisible)} 
      />
